  ...
  return (
    <header className="mb-1.5 relative border-b-2 border-black pb-1 text-black">
      ...
      <div className="mt-1 text-center">
        <div className="inline-block text-xs font-bold text-red-600 border border-red-600 px-3 py-0.5 rounded shadow-xs">
              ...
              <div className="space-y-0.5 text-xs">
                <div className="flex items-center justify-between border-b border-black/20 pb-0.5 flex-wrap gap-1">
                  <span className="font-black text-xs uppercase tracking-wide">
                    {isEn ? 'SOLUTION SHEET GUIDELINES' : 'উত্তরপত্র নির্দেশাবলী'}
                  </span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-y-0.5 text-[11px] pt-0.5 leading-snug">
                  <div>
                    ...
                    {(parsedSubjectsConfig?.mandatoryCount || 0) > 0
                      ? (isEn
                        ? `All ${parsedSubjectsConfig.mandatoryCount} mandatory subject solutions.`
                  ...
                  <div>
                    ...
                    {(parsedSubjectsConfig?.optionalCount || 0) > 0
                      ? (isEn
                        ? `Includes all ${parsedSubjectsConfig.optionalCount} optional subjects.`
                        : `মোট ${toBengaliNumerals(parsedSubjectsConfig.optionalCount)}টি ঐচ্ছিক বিষয়ের সমাধান অন্তর্ভুক্ত।`)
                  ...
                  return orderedObjective.map((q: any, idx: number) => {
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                              {parts.map((part: any, pIdx: number) => (
                                <div key={pIdx} className="p-2 border border-emerald-200 bg-white rounded text-xs">
                                  <span className="font-bold text-emerald-900"><UniversalMathJax inline dynamic>{cleanupMath(part.label || part.prompt || part.text || `Part ${pIdx+1}`)}</UniversalMathJax>: </span>
                                  <span className="font-mono font-bold text-emerald-700">
                                    <UniversalMathJax inline dynamic>{cleanupMath(String(part.expectedAnswer ?? part.modelAnswer ?? 'N/A'))}</UniversalMathJax>
                                  </span>
                    ...
                    if (q.type?.toUpperCase() === 'MPC') {
                      ...
                      return (
                        <div key={idx} className="mb-6 text-left question-block break-inside-avoid">
                          <div className="flex justify-between items-end mb-2 border-b border-black/10 pb-0.5">
                            <span className="font-bold text-sm">{qNum}. <UniversalMathJax inline dynamic>{cleanupMath(q.questionText || q.scenario || (isEn ? 'Multi-Step Problem Chain Solution' : 'বহু-ধাপী সমস্যা শৃঙ্খল সমাধান'))}</UniversalMathJax></span>
                            <span className="ml-4 font-bold text-xs">[{isEn ? (q.marks || 1) : toBengaliNumerals(q.marks || 1)}]</span>
                          </div>
                            ...
                            <div className="space-y-2">
                              {stages.map((stage: any, sIdx: number) => (
                                <div key={sIdx} className="p-2 border border-indigo-200 bg-white rounded text-xs flex justify-between items-center">
                                  <div>
                                    <span className="font-bold text-indigo-900">Stage {sIdx+1}: <UniversalMathJax inline dynamic>{cleanupMath(stage.stageTitle || stage.prompt || stage.text || '')}</UniversalMathJax></span>
                                    {stage.formula && <span className="ml-2 font-mono text-[10px] text-indigo-600">(Formula: <UniversalMathJax inline dynamic>{cleanupMath(stage.formula)}</UniversalMathJax>)</span>}
                                  </div>
                                  <span className="font-mono font-bold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded">
                                    <UniversalMathJax inline dynamic>{cleanupMath(String(stage.expectedAnswer ?? stage.modelAnswer ?? 'N/A'))}</UniversalMathJax>
                                  </span>
                                </div>
                              ))}
                            </div>
